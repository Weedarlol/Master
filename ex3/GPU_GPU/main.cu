...
void initialization(int width, int height, int depth, int iter, double dx, double dy, double dz, int gpus, int nodes, int compare, int overlap, int test, dim3 blockDim, dim3 gridDim){
    /*
    ...
    */
    ...
    if(gpus < 2){
        ...
        if(overlap == 1){
            if(test == 0){
                full_calculation_overlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollEdge, kernelCollMid);
            }
            else if(test == 1){
                no_kernel_overlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollEdge, kernelCollMid);
            }
            else if(test == 2){
                no_communication_overlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollEdge, kernelCollMid);
            }
            else if(test == 3){
                only_calculation_overlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollEdge, kernelCollMid);
            }
            else if(test == 4){
                only_communication_overlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollEdge, kernelCollMid);
            }
        }
        else{
            if(test == 0){
                full_calculation_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollMid);
            }
            else if(test == 1){
                no_kernel_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollMid);
            }
            else if(test == 2){
                no_communication_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollMid);
            }
            else if(test == 3){
                only_calculation_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollMid);
            }
            else if(test == 4){
                only_communication_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, slices_device, gridDim, blockDim, kernelCollMid);
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    initialization(width, height, depth, iter, dx, dy, dz, gpus, nodes, compare, overlap, test, blockDim, gridDim);
