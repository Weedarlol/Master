    ...
    if(gpus < 2){
        ...
        if(overlap == 1){
            ...
            if(test == 0){
                ...
                only_computation_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, rank, size, slices_device, gridDim, blockDim, kernelCollMid);
            }
            else if(test == 4){
                only_commmunication_nooverlap(data_gpu, data_gpu_tmp, width, height, depth_node, iter, gpus, rank, size, slices_device, gridDim, blockDim, kernelCollMid);
            }
        }
    }
