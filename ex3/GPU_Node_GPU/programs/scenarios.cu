...
void full_calculation_overlap(double **data_gpu, double **data_gpu_tmp, int width, int height, int depth, int iter, int gpus, int rank, int size, int *slices_device, dim3 gridDim, dim3 blockDim, void*** kernelCollEdge, void*** kernelCollMid){
    ...
    while(iter > 0){
        ...
        if(rank == 0){
            // Send data fra GPU -1 til GPU 0 på rank 1
        }
        else if(rank == size-1){
            // Send data fra GPU 0 til GPU -1 på rank size-1
        }
        else{
            // Send data fra GPU 0 til GPU -1 på rank rank-1
            // Send data fra GPU -1 til GPU 0 på rank rank+1
        }
